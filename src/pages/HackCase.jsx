...
- Missing/moved assets: ${form.missingAssets || 'Not specified'}
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            ...
            <div className="flex gap-1">
              {['EU', 'Global'].map(t => (
                <button key={t} onClick={() => update('team', t)}
                  className={cn('flex-1 text-sm py-2 rounded-lg border transition-all',
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Hack type</label>
            <div className="flex gap-1">
              {[{ val: 'exchange', label: 'Exchange' }, { val: 'web3', label: 'Web3' }].map(o => (
                <button key={o.val} onClick={() => update('hackType', o.val)}
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">VIP level</label>
            <select value={form.vipLevel} onChange={e => update('vipLevel', e.target.value)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-slate-500 mb-1 block">When did it happen?</label>
            <input value={form.incidentTime} onChange={e => update('incidentTime', e.target.value)}
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Missing / moved assets</label>
            <input value={form.missingAssets} onChange={e => update('missingAssets', e.target.value)}
              placeholder="e.g. 500 USDT withdrawn, BTC transfer..."
              className="w-full bg-slate-800 border border-slate-700 focus:border-yellow-400/50 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-600 outline-none" />
          </div>
          <div>
            <label className="text-xs text-slate-500 mb-1 block">2FA status at time</label>
            <input value={form.twoFaStatus} onChange={e => update('twoFaStatus', e.target.value)}
              placeholder="e.g. GA active, SMS only, disabled..."
              className="w-full bg-slate-800 border border-slate-700 focus:border-yellow-400/50 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-600 outline-none" />
          </div>
          <div>
            <label className="text-xs text-slate-500 mb-1 block">API keys on account?</label>
            <input value={form.apiKeys} onChange={e => update('apiKeys', e.target.value)}
        ...
        <div>
          <label className="text-xs text-slate-500 mb-1 block">Account access</label>
          <div className="flex gap-2">
            {[{ val: 'has_access', label: 'Customer can access account' }, { val: 'locked_out', label: 'Customer locked out' }].map(opt => (
              <button key={opt.val} onClick={() => update('accessStatus', opt.val)}
                className={cn('flex-1 text-sm px-3 py-2 rounded-lg border transition-all',
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
        ...
        <div className="space-y-2 pt-1">
          {SOP_STEPS.map(step => (
            <button key={step.key} onClick={() => tick(step.key)} className="flex items-center gap-3 w-full text-left group">
              {checked[step.key]
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => copy(larkTemplate, 'lark-preview')} className="flex items-center gap-1 text-xs text-slate-500 hover:text-yellow-400 transition-colors">
      ...
      <button onClick={generate} disabled={loading || !canGenerate}
        className="w-full bg-yellow-400 hover:bg-yellow-300 disabled:bg-slate-800 disabled:text-slate-600 text-slate-900 font-semibold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
